document.addEventListener("DOMContentLoaded", function() {
    ...
    // Bổ sung lắng nghe sự kiện kết thúc video Huế 1 để chuyển sang video Huế 2
    videoHue1.addEventListener('ended', function() {
        videoHue1.style.display = 'none';
        videoHue2.style.display = 'block';
        videoHue2.play();
    });

    // Lắng nghe sự kiện kết thúc video Huế 2 để hiện màn hình Sự cố âm thanh / Nấu chè
    videoHue2.addEventListener('ended', function() {
        videoHue2.style.display = 'none';
        
        // Chuyển sang màn hình Sự cố âm thanh Duyệt Thị Đường
        const hueAccidentScreen = document.getElementById('hue-accident-screen');
        if (hueAccidentScreen) {
            hueAccidentScreen.style.display = 'flex';
        }
    });
   ...
   function goToNextPageAfterHue() {
    ...
    // Hàm hiển thị màn hình Cuộn thư (chặng Huế)
    function showScrollIntroScreen() {
        if (videoHue1) { videoHue1.pause(); videoHue1.style.display = 'none'; }
        if (videoHue2) { videoHue2.pause(); videoHue2.style.display = 'none'; }
        if (videoHueNhaNhac) { videoHueNhaNhac.pause(); videoHueNhaNhac.style.display = 'none'; }

        if (scrollIntroScreen) {
            scrollIntroScreen.style.display = 'flex';
        }
    ...
    // Xử lý khi kết thúc video Nhã Nhạc / Duyệt Thị Đường
    if (videoHueNhaNhac) {
        videoHueNhaNhac.addEventListener('ended', function() {
            showScrollIntroScreen();
        });
    }
    if (btnSkipNhaNhac) btnSkipNhaNhac.addEventListener('click', goToNextPageAfterHue);
    if (videoHueNhaNhac) {
        videoHueNhaNhac.addEventListener('ended', function() {
            // 1. Dừng và ẩn video Nhã Nhạc
            videoHueNhaNhac.pause();
            videoHueNhaNhac.style.display = 'none';

            // 2. Tìm thẻ <img> chứa ảnh chặng Huế (thường là imgHue hoặc hueImage)
            const hueImg = document.getElementById('hueImage') || document.getElementById('imgHue');
            
            if (hueImg) {
                // Đổi đường dẫn sang ảnh Sự cố tại Duyệt Thị Đường
                hueImg.src = 'assets/images/3.Su_co_tai_Duyet_Thi_Duong.jpg'; // Cập nhật đúng đường dẫn thư mục ảnh của bạn
                hueImg.style.display = 'block';
            }

            // 3. Hiển thị lại khung chứa hình ảnh (nếu trước đó bị ẩn)
            const hueImageContainer = document.getElementById('hueImageContainer');
            if (hueImageContainer) {
                hueImageContainer.style.display = 'block';
            }
        });
    }
    ...
    const btnSubmitCipher = document.getElementById('btn-submit-cipher');
    const hueCongratsScreen = document.getElementById('hue-congrats-screen');

    const btnGoToHueBadge = document.getElementById('btn-go-to-hue-badge');
    const hueBadgePopupOverlay = document.getElementById('hue-badge-popup-overlay');
    const btnHueBadgeDownload = document.getElementById('btn-hue-badge-download');
    const btnHueBadgeClose = document.getElementById('btn-hue-badge-close');

    // Tự động in hoa khi gõ
    if (cipherInput) {
    ...
    // Xử lý nộp đáp án Mật thư
    if (btnSubmitCipher) {
        btnSubmitCipher.addEventListener('click', function() {
            if (!cipherInput) return;
            
            const userInput = cipherInput.value.trim();
            
            if (userInput === "VÙNG ĐẤT THUẬN HÓA PHÚ XUÂN") {
                alert("🎉 BẠN ĐÃ GIẢI MÃ CHÍNH XÁC!");
                
                // Ẩn màn giải mã
                if (scrollDetailScreen) {
                    scrollDetailScreen.style.setProperty('display', 'none', 'important');
                }
                
                // Hiển thị màn chúc mừng nhận huy hiệu
                if (hueCongratsScreen) {
                    hueCongratsScreen.style.setProperty('display', 'flex', 'important');
                    hueCongratsScreen.style.zIndex = '99999';
                }
            } else {
                alert("❌ Mật mã chưa chính xác. Hãy kiểm tra lại bảng giải mã Semaphore nhé!");
            }
        });
    }
    ...
    // Mở Popup Huy Hiệu
    if (btnGoToHueBadge) {
        btnGoToHueBadge.addEventListener('click', function() {
            if (hueCongratsScreen) hueCongratsScreen.style.display = 'none';
            if (hueBadgePopupOverlay) {
                hueBadgePopupOverlay.style.setProperty('display', 'flex', 'important');
                hueBadgePopupOverlay.style.zIndex = '999999';
            }
        });
    }

    // Tải & Đóng Huy Hiệu
    if (btnHueBadgeDownload) {
        btnHueBadgeDownload.addEventListener('click', function() {
            const img = document.getElementById('hue-badge-display-img');
            if (img) {
                const link = document.createElement('a');
                link.href = img.src;
                link.download = 'Huy_Hieu_Hue_Capy.png';
                link.click();
            }
        });
    }
